...
function getTotalCost(items){
    ...
    document.querySelector(".total-cost-numbers").innerText = numeral(totalCost).format('$0,0.00');
}
...
function generateCartItems(cartItems){
    ...
    cartItems.forEach((item)=>{
        itemsHTML+=`
        <div class="card-item flex items-center pb-4 border-b border-gray-100">
            <div class="card-item-image w-40 h-24 bg-white p-4 rounded-lg">
                <img class="w-full h-full object-contain" src="${item.image}">
            </div>
            <div class="card-item-details  flex-grow ">
                <div class="cart-item-title font-bold text-sm text-gray-600">
                    ${item.name}
                </div>
                <div class="card-item-brand text-sm text-gray-400">
                    ${item.make}
                </div>
            </div>
            <div class="card-item-counter w-48 flex items-center" >
